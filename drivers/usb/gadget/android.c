		...
		struct usb_composite_dev *cdev)
{
	...
	gbl_cdev = cdev;
	ret = mtp_setupS();
	pr_alert("MTP-NEGALITE_initS - type:%d ret:%d", is_samsung_mtp, ret);
	ret = mtp_setupG();
	pr_alert("MTP-NEGALITE_initG - type:%d ret:%d", is_samsung_mtp, ret);
...
{
	pr_alert("MTP-NEGALITE_cleanup - type:%d", is_samsung_mtp);
	if (is_samsung_mtp)
		mtp_cleanupS();
	else
		mtp_cleanupG();
		...
		struct usb_configuration *c)
{
	...
	pr_alert("MTP-NEGALITE_mtp_bind_config - type:%d ret:%d", is_samsung_mtp, ret);
	return ret;
}
...
static int ptp_function_bind_config(struct android_usb_function *f, struct usb_configuration *c)
{
	...
	pr_alert("MTP-NEGALITE_ptp_bind_config - type:%d ret:%d", is_samsung_mtp, ret);
	return ret;
}
...
{
	struct android_dev *dev = cdev_to_android_dev(c->cdev);
	pr_alert("MTP-NEGALITE_unbind_config - type:%d", is_samsung_mtp);
...
{
	is_samsung_mtp = val;
	pr_alert("MTP-NEGALITE_set_mtp_type - type:%d", is_samsung_mtp);
}
