...
#hard reset control
chown radio.system /sys/power/hard_reset_ctl
chmod 664 /sys/power/hard_reset_ctl
...
if [ $SD_MEMORY_CONTROL = "On" ]; then
	...
	echo $VIR_READ_AHEAD_KB > /sys/devices/virtual/bdi/253:1/read_ahead_kb
	echo $VIR_READ_AHEAD_KB > /sys/devices/virtual/bdi/253:2/read_ahead_kb
	echo $VIR_READ_AHEAD_KB > /sys/devices/virtual/bdi/253:3/read_ahead_kb
...
# Journaling
tune2fs -o journal_data_writeback /dev/block/mmcblk0p21
tune2fs -o journal_data_writeback /dev/block/mmcblk0p22
